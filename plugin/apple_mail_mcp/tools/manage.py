...
@mcp.tool()
@inject_preferences
def evict_archived_from_inbox(
    account: str,
    internet_message_ids: List[str],
    archive_mailbox: str = "Archive",
) -> str:
    """
    Drop already-archived messages from Mail.app's LOCAL inbox cache.

    Use this AFTER a message has been archived through a direct-IMAP path that
    strips the server-side INBOX label WITHOUT going through Mail.app. In that
    case Mail.app's local INBOX copy lingers — sometimes for a long time —
    until Mail.app's own sync notices the message left the inbox, so the Mac
    keeps showing "archived" mail in the inbox. This forces the local catch-up
    by moving any lingering INBOX copy to the Archive mailbox, matched by RFC
    822 Message-ID (stable across Mail's numeric-ROWID reshuffles).

    The server is already archived, so this is a LOCAL reconcile only: it never
    deletes and never moves to Trash, and the move target is the real Archive
    folder (never All Mail). Message-IDs not present in INBOX (already evicted,
    or never there) are counted as "already gone", not errors — the operation
    is idempotent and safe to call speculatively.

    Args:
        account: Account name (e.g. "Gunpowder")
        internet_message_ids: RFC 822 Message-IDs (with or without angle
            brackets) of the messages to evict from the local INBOX.
        archive_mailbox: Destination mailbox the messages were archived into
            (default: "Archive"). Used as the local move target.

    Returns:
        A summary line with REQUESTED / EVICTED / ALREADY GONE counts (plus
        FAILED VERIFY if a local move silently no-op'd).
    """

    safe_account = escape_applescript(account)

    # Normalize: strip angle brackets + whitespace. Mail's `message id`
    # property returns the bare value on modern Mail.app; we also try a
    # bracketed form per message as a cross-version fallback (mirrors
    # reply_to_email_by_id in compose.py).
    normalized: List[str] = []
    for raw in internet_message_ids or []:
        v = (raw or "").strip().strip("<>").strip()
        if v:
            normalized.append(v)
    if not normalized:
        return "Error: 'internet_message_ids' must contain one or more RFC Message-IDs"

    id_list_literal = "{" + ", ".join(
        f'"{escape_applescript(v)}"' for v in normalized
    ) + "}"
    safe_archive = escape_applescript(archive_mailbox)

    # For each target Message-ID: find the lingering INBOX copy, move it to
    # Archive, then residual-verify it actually left INBOX (the same silent-
    # no-op guard move_email uses). A message that isn't in INBOX is a no-op.
    script = f'''
    tell application "Mail"
        with timeout of 300 seconds
            set evictedCount to 0
            set goneCount to 0
            set residualCount to 0
            try
                set targetAccount to account "{safe_account}"
                ...
                repeat with rawMid in {id_list_literal}
                    ...
                    set candidates to (every message of sourceMailbox whose message id is targetMid)
                    if (count of candidates) is 0 then
                        set candidates to (every message of sourceMailbox whose message id is targetMidBracketed)
                    end if
                    if (count of candidates) is 0 then
                        set goneCount to goneCount + 1
                    else
